...
public interface TurnoRepositorio extends CrudRepository<TurnoEntidad, Long> {
    ...
    @Query("select t from TurnoEntidad t where t.profesionalEntidad.especialidad.id = ?1")
    List<TurnoEntidad> findByEspecialidad(Long id);

    @Query("select t from TurnoEntidad t where t.profesionalEntidad.id = ?1")
    List<TurnoEntidad> findByProfesional(Long id);
}
